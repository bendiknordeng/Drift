...
final class NeonService {
    ...
    func fetchOrganizations(apiKey overrideAPIKey: String? = nil) async throws -> [NeonOrganization] {
        let data = try await request(path: "/users/me/organizations", apiKey: overrideAPIKey)
        let wrapper = try JSONDecoder().decode(OrganizationsResponse.self, from: data)
        return wrapper.organizations
    }

    func fetchProjects(orgId: String?, apiKey overrideAPIKey: String? = nil) async throws -> [NeonProject] {
        var path = "/projects"
        if let orgId {
            path += "?org_id=\(orgId)"
        }
        let data = try await request(path: path, apiKey: overrideAPIKey)
        let wrapper = try JSONDecoder().decode(ProjectsResponse.self, from: data)
        return wrapper.projects
    }

    func fetchBranches(projectId: String, apiKey overrideAPIKey: String? = nil) async throws -> [NeonBranch] {
        let data = try await request(path: "/projects/\(projectId)/branches", apiKey: overrideAPIKey)
        let wrapper = try JSONDecoder().decode(BranchesResponse.self, from: data)
        return wrapper.branches
    }

    func fetchDatabases(projectId: String, branchId: String, apiKey overrideAPIKey: String? = nil) async throws -> [NeonDatabase] {
        let data = try await request(path: "/projects/\(projectId)/branches/\(branchId)/databases", apiKey: overrideAPIKey)
        let wrapper = try JSONDecoder().decode(DatabasesResponse.self, from: data)
        return wrapper.databases
    }

    func fetchRoles(projectId: String, branchId: String, apiKey overrideAPIKey: String? = nil) async throws -> [NeonRole] {
        let data = try await request(path: "/projects/\(projectId)/branches/\(branchId)/roles", apiKey: overrideAPIKey)
        let wrapper = try JSONDecoder().decode(RolesResponse.self, from: data)
        return wrapper.roles
    }

    func fetchConnectionURI(
        projectId: String,
        branchId: String,
        database: String,
        role: String,
    ...
    private func request(path: String, apiKey overrideAPIKey: String? = nil) async throws -> Data {
        let requestAPIKey = (overrideAPIKey ?? apiKey).trimmingCharacters(in: .whitespacesAndNewlines)
        guard !requestAPIKey.isEmpty else { throw DriftError.neonAPIError("API key not configured") }

        var req = URLRequest(url: URL(string: baseURL + path)!)
        req.setValue("Bearer \(requestAPIKey)", forHTTPHeaderField: "Authorization")
